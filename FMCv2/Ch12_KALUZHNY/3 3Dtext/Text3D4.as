class Text3D4 extends MovieClip {
	...
	private function makeShape(u:Number, v:Number, p:Object):Void {
		var un = (v-10)/13;
		p.x = (u-20)*4 * Math.cos(Math.cos(un));
		p.z = 50+(u-20)*4 * Math.sin(Math.cos(un));
		p.y = (16-v)*4;
	}
	...
	function onEnterFrame (Void):Void {
		...
		for (var i=0; i < numChars; i++ ) {
			...
			var c:Number = int(78 + pnt0.z *1.3) & 0xFF;
