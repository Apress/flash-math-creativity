class Ball2 extends MovieClip {
	...
	private function init(){
		...
		for (var i=0; i<len; i++) {
			...
			ring.ix = rang * Math.cos(i * 2 * Math.PI / len);
			ring.ix1 = (rang + 1) * Math.cos(i * 2 * Math.PI / len);
			ring.ix2 = rang * Math.cos(i * 2 * Math.PI / len);
			ring.iy = rang * Math.sin(i * 2 * Math.PI / len);
			ring.iy1 = (rang + 1) * Math.sin(i * 2 * Math.PI / len);
			ring.iy2 = rang * Math.sin(i * 2 * Math.PI / len);
			ring.iz = 0;
			ring.iz1 = 0;
			ring.iz2 = 1;
		}
